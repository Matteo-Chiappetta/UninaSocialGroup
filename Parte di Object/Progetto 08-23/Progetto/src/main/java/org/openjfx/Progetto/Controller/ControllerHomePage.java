...
public class ControllerHomePage implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		/*
		 * if(UtenteDao.getUtenteCorrente().getImgProfilo() == null ||
		 * UtenteDao.getUtenteCorrente().getImgProfilo().length() < 10) { File file =
		 * new File("src\\main\\resources\\org\\openjfx\\Immagine\\utente.jpg"); Image
		 * image = new Image(file.toURI().toString()); imgProfilo.setImage(image); }else
		 * { File file = new File(UtenteDao.getUtenteCorrente().getImgProfilo()); Image
		 * image = new Image(file.toURI().toString()); imgProfilo.setImage(image); }
		 */
